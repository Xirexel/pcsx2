...
static u8 __pagealigned eeRecDispatchers[__pagesize * 16];
...
static void _DynGen_Dispatchers()
{
	...
	setNOP( eeRecDispatchers, 0xcc, __pagesize * 16);
...
{
	if (!recMem) recMem = new RecompiledCodeReserve(L"R5900-32 Recompiler Cache", _64mb);
...
static void iBranchTest(u32 newpc)
{
	...
	else
	{
		...
		if (newpc == 0xffffffff)
			...
			recBlocks.Link(HWADDR(newpc), JS32(Jcc_Signed));
