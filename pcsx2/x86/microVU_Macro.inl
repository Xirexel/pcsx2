...
void _setupBranchTest(bool isLikely) {
	printCOP2("COP2 Branch");
	_eeFlushAllUnused();
	xTEST(ptr32[&vif1Regs.stat._u32], 0x4);
	//TEST32ItoM((uptr)&VU0.VI[REG_VPU_STAT].UL, 0x100);
	ForwardType jump;
	recDoBranchImm((x86Emitter::xForwardJumpBase*)&jump, isLikely);
}

void recBC2F()  { _setupBranchTest<xForwardJNZ32>(false); }
void recBC2T()  { _setupBranchTest<xForwardJZ32>(false); }
void recBC2FL() { _setupBranchTest<xForwardJNZ32 >(true);  }
void recBC2TL() { _setupBranchTest<xForwardJZ32 >(true);  }
