...
const u8* ARM64XEmitter::AlignCode16()
{
	...
	if (c)
    {
//        m_code = (u8 *)(((uptr)m_code + c - 1) & ~(c - 1));
//        m_writable = (u8 *)(((uptr)m_writable + c - 1) & ~(c - 1));
...
void ARM64XEmitter::SetJumpTarget(FixupBranch const& branch, s64 distance)
{
	if(branch.bit == 8)
	{
		bool Not = false;
		u32 inst = 0;
		if(distance == 0)
			distance = (s64)(m_code - branch.ptr);

		auto originaldistance = distance;

		distance >>= 2;

		if(originaldistance != (distance << 2))
        {
            throw L"Unimplemented!!!";
	...
	} else if(branch.bit == 32)
	{
        auto l_current_write_memory = g_Emitter.GetWritableCodePtr();
        g_Emitter.SetCodePointer((u8 *)branch.ptr, (u8 *)branch.ptr);

        if(distance == 0)
			distance = (s64)l_current_write_memory;

		auto offsetdistance = distance >> 2;
        ...
        switch (branch.type)
        {
            ...
            case 2: // B (conditional)
            {
                g_Emitter.MOVI2R(Arm64JitConstants::SCRATCH1_64, (u64)distance - 16);
                g_Emitter.ADR(Arm64JitConstants::SCRATCH2_64, 0);
                g_Emitter.CSEL(Arm64JitConstants::SCRATCH2_64, Arm64JitConstants::SCRATCH1_64, Arm64JitConstants::SCRATCH2_64, branch.cond);
                g_Emitter.ADD(Arm64JitConstants::SCRATCH2_64, Arm64JitConstants::SCRATCH2_64,16);
                g_Emitter.BR(Arm64JitConstants::SCRATCH2_64);
            }
                break;
            case 4: // TBNZ
                throw L"Unimplemented!!!";
            ...
            case 5: // B (unconditional)
            {
                g_Emitter.MOVI2R(Arm64JitConstants::SCRATCH1_64, (u64)distance);
        ...
        g_Emitter.SetCodePointer(l_current_write_memory, l_current_write_memory);
	}
	else
		throw L"Unimplemented!!!";
}
...
FixupBranch ARM64XEmitter::B(CCFlags cond, u8 bit)
{
	FixupBranch branch;
	branch.ptr = m_code;
	branch.type = 2;
...
FixupBranch ARM64XEmitter::B(u8 bit)
{
	FixupBranch branch;
	branch.ptr = m_code;
	branch.type = 5;
...
void ARM64XEmitter::MOV(ARM64Reg Rd, ARM64Reg Rm)
{
	if (IsGPR(Rd) && IsGPR(Rm)) {
		ORR(Rd, Is64Bit(Rd) ? ZR : WZR, Rm, ArithOption(Is64Bit(Rm) ? ZR : WZR, ST_LSL, 0));
	} else {
...
void ARM64XEmitter::STR(ARM64Reg Rt, ARM64Reg Rn)
{
	bool b64Bit = Is64Bit(Rt);
	EncodeLoadStoreRegisterOffset(2 + b64Bit, 0, Rt, Rn, ArithOption(ZR));
}
...
void ARM64XEmitter::LDR(ARM64Reg Rt, ARM64Reg Rn)
{
	bool b64Bit = Is64Bit(Rt);
	EncodeLoadStoreRegisterOffset(2 + b64Bit, 1, Rt, Rn, ArithOption(ZR));
}
...
{
	EncodeLoadStoreRegisterOffset(size, false, Rt, Rn, ArithOption(ZR));
}
...
{
	EncodeLoadStoreRegisterOffset(size, true, Rt, Rn, ArithOption(ZR));
}
