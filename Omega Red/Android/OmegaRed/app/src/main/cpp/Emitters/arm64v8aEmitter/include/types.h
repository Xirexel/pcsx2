...
#define __fi //__attribute__((always_inline))
...
#define MEMORY_DATA_TYPE DataType::Double_Word
#else
#ifdef __M_X86_64
#define MEMORY_DATA_TYPE DataType::Double_Word
#else
#define MEMORY_DATA_TYPE DataType::Word
#endif
#endif
...
extern void setNOP(void* ptrMem, int value, u32 byteSize);
...
public:
    ...
    DataType DataType = MEMORY_DATA_TYPE;

public:
    xAddressVoid(const xAddressReg &base, const xAddressReg &index, int factor = 1, sptr displacement = 0, x86Emitter::DataType dataType = MEMORY_DATA_TYPE);
...
public:
    ...
    void setType(const OperandType *addr)
    {
        auto l_var_size = sizeof(OperandType);

        if(l_var_size == 1)
        {
            DataType = DataType::Byte;
        } else if(l_var_size == 2)
        {
            DataType = DataType::Half_Word;
        } else if(l_var_size == 4)
        {
            DataType = DataType::Word;
        } else if(l_var_size == 8)
        {
            DataType = DataType::Double_Word;
        }
    }

    template <typename OperandType>
    xAddressVoid &Add(const OperandType *addr)
    {
        setType(addr);
        Displacement += (sptr)addr;
        return *this;
    }
...
public:
    explicit xIndirectVoid(sptr disp, uint a_byteSize);
    explicit xIndirectVoid(const xAddressVoid &src);
    xIndirectVoid(xAddressReg base, xAddressReg index, int scale = 0, sptr displacement = 0, x86Emitter::DataType dataType = MEMORY_DATA_TYPE);

    template <typename OperandType>
    void setType(const OperandType *addr)
    {
        DataType = setType(sizeof(OperandType));
    }

    x86Emitter::DataType setType(uint a_byteSize)
    {
        x86Emitter::DataType dataType = MEMORY_DATA_TYPE;

        if(a_byteSize == 1)
        {
            dataType = DataType::Byte;
        } else if(a_byteSize == 2)
        {
            dataType = DataType::Half_Word;
        } else if(a_byteSize == 4)
        {
            dataType = DataType::Word;
        } else if(a_byteSize == 8)
        {
            dataType = DataType::Double_Word;
        }
...
public:
    explicit xIndirect(sptr disp, uint aByteSize)
            : _parent(disp, GetOperandSize())
    {
    }
...
protected:
    ...
    explicit xIndirect64orLess(sptr disp, uint aByteSize)
        : _parent(disp, aByteSize)
    {
    }
...
public:
    ...
    template <typename OperandType>
    xModSibType operator[](const OperandType *src) const
    {
        return xModSibType((sptr)src, sizeof(OperandType));
    }

    xModSibType operator[](const void *src) const
    {
        return xModSibType((sptr)src, sizeof(sptr));
    }
};
...
{
    FixupBranchJump _fixupBranchJump;


public:
    xForwardJumpBase(uint opsize, JccComparisonType cctype);

    virtual void SetTarget() const = 0;

    virtual void SetTarget(uptr target) const = 0;

    virtual void SetRelTarget(sptr relTarget) const = 0;

protected:
    void _setTarget(uint opsize) const;
    void _setTarget(uptr target, uint opsize) const;
    void _setTarget(sptr target, uint opsize) const;
...
public:
    ...
    virtual void SetTarget() const
    {
        _setTarget(OperandSize);
    }

    virtual void SetTarget(uptr target) const
    {
        _setTarget(target, OperandSize);
    }

    virtual void SetRelTarget(sptr relTarget) const
    {
        _setTarget(relTarget, OperandSize);
    }
};
