...
#pragma warning(pop)
	...
	void *modifiedmemcpy(void *dst, void const *src, size_t len)
	{
		void *r = dst;

		//Copy, the length of the uint64_t in general, memory addresses are aligned,
		size_t n = len & ~(sizeof(uint64_t) - 1);
		...
		//Part copy will not 8 byte length integer
        n = len & (sizeof(uint64_t) - 1);
	...
	memcpy MemoryManager::memcpy = &apex::modifiedmemcpy;
	
	memmove MemoryManager::memmove = &apex::modifiedmemcpy;
	...
	#define bit_SSE4_2 (1 << 20)
			...
			if (cpuid[2] & bit_SSE4_2)         //	detect SSE4.2, available on Core i and newer processors, they include "fast unaligned" memory access
				MemoryManager::memcpy = MemoryManager::memmove = &apex::kryptonite;
			else if (cpuid[2] & bit_SSSE3)
				MemoryManager::memcpy = MemoryManager::memmove = &apex::tiberium;
	#else //	32-bit
			if (cpuid[2] & bit_SSE4_2) //	detect SSE4.2, available on Core i and newer processors, they include "fast unaligned" memory access
				MemoryManager::memcpy = MemoryManager::memmove = &apex::kryptonite;
			else if (cpuid[2] & bit_SSSE3)                                        //	detect SSSE3, available on Core/Core 2 and newer
				MemoryManager::memcpy = MemoryManager::memmove = &apex::tiberium; //	`tiberium` CAN run with the SSE2 instructions alone, however there was something about very old SSE2 (only) computers, some penalty when using `tiberiums` algorithm, so I've restricted `tiberium` to Core/Core 2 based machines on 32-bit by doing this!
			else if (cpuid[3] & bit_SSE2)
				MemoryManager::memcpy = MemoryManager::memmove = &apex::mithril; //	this is for very, very old computers with SSE2 only! eg. old Pentium 4! There was something about computers WITHOUT SSSE3 (Core microarchitecture) that made `tiberium` a bit slower. So this is for super old (P4) PC's!
	#endif
	#endif
