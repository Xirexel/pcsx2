...
Device::Device(DeviceAPI api, DeviceType d, const wchar_t *displayName, const wchar_t *instanceID, const wchar_t *productID)
{
    ...
    this->api = api;
    type = d;
    this->displayName = _wcsdup(displayName);
    if (instanceID)
		this->instanceID = _wcsdup(instanceID);
    else
		this->instanceID = _wcsdup(displayName);
    this->productID = 0;
    if (productID)
		this->productID = _wcsdup(productID);
    active = 0;
    attached = 1;
    enabled = 0;
...
void Device::AddFFEffectType(const wchar_t *displayName, const wchar_t *effectID, EffectType type)
{
	...
	ffEffectTypes[numFFEffectTypes].displayName = _wcsdup(displayName);
	ffEffectTypes[numFFEffectTypes].effectID = _wcsdup(effectID);
...
void Device::AddFFAxis(const wchar_t *displayName, int id)
{
	...
	ffAxes[numFFAxes].displayName = _wcsdup(displayName);
    numFFAxes++;
    int bindingsExist = 0;
    for (int port = 0; port < 2; port++) {
        for (int slot = 0; slot < 4; slot++) {
...
ForceFeedbackEffectType *Device::GetForcefeedbackEffect(wchar_t *id)
{
    for (int i = 0; i < numFFEffectTypes; i++) {
		if (!_wcsicmp(id, ffEffectTypes[i].effectID)) {
