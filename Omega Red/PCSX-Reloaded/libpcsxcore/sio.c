				...
				if (rdwr == 2) {
					switch (CtrlReg & 0x2002) {
						case 0x0002:
							...
							SaveMcd(0, Mcd1Data, (adrL | (adrH << 8)) * 128, 128);
							break;
						case 0x2002:
							memcpy(Mcd2Data + (adrL | (adrH << 8)) * 128, &buf[1], 128);
							SaveMcd(1, Mcd2Data, (adrL | (adrH << 8)) * 128, 128);
							break;
					}
				}
			}
...
void LoadMcd(int mcd, char *str) {
	...
	readFromMcd(mcd - 1, data, 0, MCD_SIZE);
...
void SaveMcd(int port, char *data, uint32_t adr, int size) {
		
    writeToMcd(port, data + adr, adr, size);
