...
void psxBios_format()
{ // 0x41
    if (strcmp(Ra0, "bu00:") == 0 && isMcdPresented(0) != FALSE) {

		formatMcd(0);

        LoadMcd(1, Config.Mcd1);
        v0 = 1;
    } else if (strcmp(Ra0, "bu10:") == 0 && isMcdPresented(1) != FALSE) {
...
{ // ab
    ...
    boolean nocard = isMcdPresented(port) == FALSE;
#ifdef PSXBIOS_LOG
...
{ // ac
    ...
    if (port == 0) {
        nocard = readFromMcd(port, Mcd1Data, 0, MCD_SIZE) == FALSE;
	}
	else{
        nocard = readFromMcd(port, Mcd2Data, 0, MCD_SIZE) == FALSE;
    }
    ...
    {                                                                   \
        ...
        if (a1 & 0x200 && v0 == -1) { /* FCREAT */                      \
            for (i = 1; i < 16; i++) {                                  \
                ...
                SysPrintf("openC %s\n", ptr);                           \
                v0 = 1 + mcd;                                           \
                SaveMcd(mcd-1, Mcd##mcd##Data, 128 * i, 128); \
                break;                                                  \
            }                                                           \
        }                                                               \
    }
...
#define buwrite(mcd)                                                                      \
    {                                                                                     \
        ...
        SaveMcd(mcd-1, Mcd##mcd##Data, offset, a2);										  \
        if (FDesc[1 + mcd].mode & 0x8000)                                                 \
            v0 = 0;                                                                       \
        else                                                                              \
            v0 = a2;                                                                      \
...
{ // 3d
    ...
    if (psxstrbuf_count < PSXSTRBUFMAX)
        psxstrbuf[psxstrbuf_count++] = logchar;
    ...
    {                                                                      \
        for (i = 1; i < 16; i++) {                                         \
            ...
            for (j = 0; j < 127; j++)                                      \
                xor ^= ptr[j];                                             \
            ptr[127] = xor;                                                \
            SaveMcd(mcd - 1, Mcd##mcd##Data, 128 * i + 0xa, 0x76);		   \
            v0 = 1;                                                        \
            break;                                                         \
        }                                                                  \
    }
    ...
    {                                                             \
        for (i = 1; i < 16; i++) {                                \
            ...
            if (strcmp(Ra0 + 5, ptr + 0xa))                       \
                continue;                                         \
            *ptr = (*ptr & 0xf) | 0xA0;                           \
            SaveMcd(mcd - 1, Mcd##mcd##Data, 128 * i, 1);		  \
            SysPrintf("delete %s\n", ptr + 0xa);                  \
            v0 = 1;                                               \
            break;                                                \
        }                                                         \
...
{ // 0x4e
    ...
    if (port == 0) {
        ...
        SaveMcd(port, Mcd1Data, a1 * 128, 128);
    } else {
        memcpy(Mcd2Data + a1 * 128, Ra2, 128);
        SaveMcd(port, Mcd2Data, a1 * 128, 128);
    }
...
#define bfreeze(ptr, size)                  \
    {                                       \
        if (Mode == 1)                      \
            memcpy(&psxR[base], ptr, size); \
        if (Mode == 0)                      \
            memcpy(ptr, &psxR[base], size); \
        base += size;                       \
    }
...
#define bfreezepsxMptr(ptr, type)                             \
    {                                                         \
        if (Mode == 1) {                                      \
            if (ptr)                                          \
                psxRu32ref(base) = SWAPu32((s8 *)(ptr)-psxM); \
            else                                              \
                psxRu32ref(base) = 0;                         \
        } else {                                              \
            if (psxRu32(base) != 0)                           \
                ptr = (type *)(psxM + psxRu32(base));         \
            else                                              \
                (ptr) = NULL;                                 \
        }                                                     \
        base += sizeof(u32);                                  \
