...
LONG CurrentVolume = (((0.5 * -1) * (double)DSBVOLUME_MIN) + DSBVOLUME_MIN);

SetDataCallback g_setAudioData = NULL;
...
void DSSetVolume(LONG aVolume)
{
    ...
    if (lpDSB != NULL)
        lres = IDirectSoundBuffer_SetVolume(lpDSB, aVolume);
}
...
void SetupSound(void)
{
 ...
 dsbdesc.dwFlags = DSBCAPS_LOCSOFTWARE | DSBCAPS_STICKYFOCUS | DSBCAPS_GETCURRENTPOSITION2 | DSBCAPS_CTRLVOLUME;
...
void clearStreamData()
{
    ...
    IDirectSoundBuffer_GetStatus(lpDSB, &status);
    if (status & DSBSTATUS_BUFFERLOST) {
        if (IDirectSoundBuffer_Restore(lpDSB) != DS_OK)
            return;
        IDirectSoundBuffer_Play(lpDSB, 0, 0, DSBPLAY_LOOPING);
    }
...
*/
    ...
    DSBCAPS l_BufferCaps;

    l_BufferCaps.dwSize = sizeof(DSBCAPS);

    hr = IDirectSoundBuffer_GetCaps(lpDSB, &l_BufferCaps);

    if (hr != DS_OK)
        return;

    if (l_BufferCaps.dwBufferBytes == 0)
        return;

	DWORD lBytes = l_BufferCaps.dwBufferBytes;
...
{
    ULONGLONG l_millisecond = GetTickCount64();

    return l_millisecond; // to do that, but at least it works
}
