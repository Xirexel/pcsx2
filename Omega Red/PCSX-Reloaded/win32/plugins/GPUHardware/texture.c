...
void CheckTextureMemory(void)
{
 ...
 if(iBlurBuffer)
  {
   ...
   glTexImage2DStub(GL_TEXTURE_2D, 0, 3, iFTexA, iFTexB, 0, GL_RGB, GL_UNSIGNED_BYTE, p);
   free(p);
   //glGetError();
 ...
 for(i=0;i<MAXSORTTEX;i++)
  {
   glBindTextureStub(GL_TEXTURE_2D,uiStexturePage[i]);
   glTexImage2DStub(GL_TEXTURE_2D, 0, giWantedRGBA, iTSize, iTSize, 0,GL_RGBA, giWantedTYPE, p);
  }
 glBindTextureStub(GL_TEXTURE_2D,0);
 ...
 glDeleteTexturesStub(MAXSORTTEX,uiStexturePage);
 ...
 for(i=0;i<MAXSORTTEX;i++)
  {
   if(uiStexturePage[i])                             // --> tex used ?
    {
     glDeleteTexturesStub(1,&uiStexturePage[i]);
...
void ResetTextureArea(BOOL bDelTex)
{
 ...
 //----------------------------------------------------//
 if(bDelTex) {glBindTextureStub(GL_TEXTURE_2D,0);gTexName=0;}
 //----------------------------------------------------//
 tsx=wcWndtexStore;
 for(i=0;i<MAXWNDTEXCACHE;i++,tsx++)
  {
   ...
   if(bDelTex && tsx->texname)
    {
     glDeleteTexturesStub(1,&tsx->texname);
     tsx->texname=0;
    }
  }
 iMaxTexWnds=0;
 ...
 for(i=0;i<iSortTexCnt;i++)
  {
   ...
   if(bDelTex && uiStexturePage[i])
    {glDeleteTexturesStub(1,&uiStexturePage[i]);uiStexturePage[i]=0;}
  }
}
...
void DefineTextureWnd(void)
{
 ...
 glTexImage2DStub(GL_TEXTURE_2D, 0,giWantedRGBA, 
              TWin.Position.x1, 
              TWin.Position.y1, 
...
void DefinePalTextureWnd(void)
{
 ...
 glTexImage2DStub(GL_TEXTURE_2D, 0,GL_COLOR_INDEX8_EXT, 
              TWin.Position.x1, 
              TWin.Position.y1, 
...
void DefinePackedTextureMovie(void)
{
 if(gTexMovieName==0)
  ...
  {
   gTexName=gTexMovieName;glBindTextureStub(GL_TEXTURE_2D, gTexName);
  }

 glTexSubImage2DStub(GL_TEXTURE_2D, 0, 0, 0,
 ...
 if(gTexMovieName==0)
  {
   ...
   glTexImage2DStub(GL_TEXTURE_2D, 0, giWantedRGBA, 256, 256, 0, GL_RGBA, GL_UNSIGNED_BYTE, texturepart);
  }
 else 
  {
   gTexName=gTexMovieName;glBindTextureStub(GL_TEXTURE_2D, gTexName);
   ...
   if(!gTexFrameName)
    {
     ...
     glTexImage2DStub(GL_TEXTURE_2D, 0, giWantedRGBA, 4, 4, 0, GL_RGBA, GL_UNSIGNED_BYTE, texturepart);
    }
   else
    {
     gTexName=gTexFrameName;
...
GLuint Fake15BitTexture(void)
{
 ...
 if(!gTexFrameName)
  {
   ...
   memset(p,0x80,iFTex*iFTex*4);
   //glTexImage2DStub(GL_TEXTURE_2D, 0, 3, iFTex, iFTex, 0, GL_RGB, GL_UNSIGNED_BYTE, p);
   glTexImage2DStub(GL_TEXTURE_2D, 0, 3, iFTex, iFTex, 0, GL_BGR_EXT, GL_UNSIGNED_BYTE, p);
   free(p);
 ...
 glCopyTexSubImage2DStub(GL_TEXTURE_2D, 0,
	 0,
	 iYAdjust,
	 rSrc.left + rRatioRect.left,
	 rSrc.top - rRatioRect.top,
	 x1, y1);
 ...
 //  glTexSubImage2DStub(GL_TEXTURE_2D, 0, 0, 0, iFTex, iFTex,
...
void DefineSubTextureSortHiRes(void)
{
 ...
 if(!gTexName)             
  {
   ...
   glTexImage2DStub(GL_TEXTURE_2D, 0, giWantedRGBA, 512, 512, 0, giWantedFMT, giWantedTYPE, texturebuffer);
  }
 else glBindTextureStub(GL_TEXTURE_2D, gTexName);
 ...
 glTexSubImage2DStub(GL_TEXTURE_2D, 0, XTexS<<1, YTexS<<1,
                 DXTexS<<1, DYTexS<<1,
 ...
 if(!gTexName)
  {
   ...
   glTexImage2DStub(GL_TEXTURE_2D, 0, giWantedRGBA, 256, 256, 0,giWantedFMT, giWantedTYPE, texturepart);
  }
 else glBindTextureStub(GL_TEXTURE_2D, gTexName);

 glTexSubImage2DStub(GL_TEXTURE_2D, 0, XTexS, YTexS,
                 DXTexS, DYTexS,
                 giWantedFMT, giWantedTYPE, texturepart);
}
