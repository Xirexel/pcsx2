...
const size_t g_vertextSize = sizeof(VERTEX);

VERTEX g_border_vertexes[4];
...
void PRIMDirectXdrawBorder()
{
	...
	g_border_vertexes[0].lcol = lcol;
	g_border_vertexes[1].lcol = lcol;
	g_border_vertexes[2].lcol = lcol;
	g_border_vertexes[3].lcol = lcol;




	g_border_vertexes[0].w = 1.0f;
	g_border_vertexes[1].w = 1.0f;
	g_border_vertexes[2].w = 1.0f;
	g_border_vertexes[3].w = 1.0f;


	draw(GL_QUADS, g_border_vertexes, FALSE);
}
...
#define DEFOPAQUEON  glAlphaFuncStub(GL_EQUAL,0.0f);bBlendEnable=FALSE;glDisableStub(GL_BLEND);                                
#define DEFOPAQUEOFF glAlphaFuncStub(GL_GREATER,0.49f);
...
static __inline void PRIMDirectXdrawTexturedQuad(OGLVertex* vertex1, OGLVertex* vertex2,
	OGLVertex* vertex3, OGLVertex* vertex4)
{
	...
	
	{
		...
		memcpy(l_pDest, vertex1, g_vertextSize);
	...
	
	{
		...
		memcpy(l_pDest, vertex2, g_vertextSize);
...

	{
		...
		memcpy(l_pDest, vertex4, g_vertextSize);
	...
	
	{
		...
		memcpy(l_pDest, vertex3, g_vertextSize);
	...
	draw(GL_TRIANGLE_STRIP, temp, TRUE);
...
static __inline void PRIMdrawTexturedQuad(OGLVertex* vertex1, OGLVertex* vertex2, 
                                   OGLVertex* vertex3, OGLVertex* vertex4) 
{
 PRIMDirectXdrawTexturedQuad(vertex1, vertex2, vertex3, vertex4);
}

///////////////////////////////////////////////////////// 

static __inline void PRIMDirectXdrawTexturedTri()
{
	VERTEX temp[3];
	...
	for (size_t vertexIndex = 0; vertexIndex < 3; vertexIndex++)
	{
		...
		memcpy(l_pDest, &vertex[vertexIndex], g_vertextSize);
...
static __inline void PRIMdrawTexturedTri(OGLVertex* vertex1, OGLVertex* vertex2, 
                                  OGLVertex* vertex3) 
{
 PRIMDirectXdrawTexturedTri();
}

///////////////////////////////////////////////////////// 

static __inline void PRIMDirectXdrawTexGouraudTriColor()
{
	VERTEX temp[3];
...
static __inline void PRIMdrawTexGouraudTriColor(OGLVertex* vertex1, OGLVertex* vertex2, 
                                         OGLVertex* vertex3) 
{
 PRIMDirectXdrawTexGouraudTriColor();
}

///////////////////////////////////////////////////////// 

static __inline void PRIMDirectXdrawTexGouraudTriColorQuad()
{
	VERTEX temp[4];
...
static __inline void PRIMdrawTexGouraudTriColorQuad(OGLVertex* vertex1, OGLVertex* vertex2, 
                                             OGLVertex* vertex3, OGLVertex* vertex4) 
{
 PRIMDirectXdrawTexGouraudTriColorQuad();
}

///////////////////////////////////////////////////////// 

static __inline void PRIMDirectXdrawTri()
{
	VERTEX temp[3];
...
static __inline void PRIMDirectXdrawTri2()
{
	VERTEX temp[4];
	...
	draw(GL_TRIANGLE_STRIP, temp, FALSE);
...
static __inline void PRIMDirectXdrawGouraudTriColor()
{
	VERTEX temp[3];
...
static __inline void PRIMdrawGouraudTriColor(OGLVertex* vertex1, OGLVertex* vertex2, 
                                      OGLVertex* vertex3) 
{
 PRIMDirectXdrawGouraudTriColor();
}

///////////////////////////////////////////////////////// 

static __inline void PRIMDirectXdrawGouraudTri2Color()
{
	VERTEX temp[4];
...
static __inline void PRIMdrawGouraudTri2Color(OGLVertex* vertex1, OGLVertex* vertex2, 
                                       OGLVertex* vertex3, OGLVertex* vertex4) 
{
 PRIMDirectXdrawGouraudTri2Color();
}

///////////////////////////////////////////////////////// 

static __inline void PRIMDirectXdrawFlatLine()
{
...
{
 PRIMDirectXdrawFlatLine();
}

///////////////////////////////////////////////////////// 

static __inline void PRIMDirectXdrawGouraudLine()
{
	VERTEX temp[4];
...
{
 PRIMDirectXdrawGouraudLine();
}

///////////////////////////////////////////////////////// 

static __inline void PRIMDirectXdrawQuad()
{
	VERTEX temp[4];
...
*/
 ...
 if(!DrawSemiTrans)                                    // no semi trans at all?
  {
   if(bBlendEnable)
    {glDisableStub(GL_BLEND);bBlendEnable=FALSE;}          // -> don't wanna blend
 ...
 if(!bBlendEnable)
  {glEnableStub(GL_BLEND);bBlendEnable=TRUE;}              // wanna blend
...
void SetSemiTransMulti(int Pass)
{
 ...
 if(bm1!=obm1 || bm2!=obm2)
  {
   glBlendFuncStub(bm1,bm2);                            // set blend func
   obm1=bm1;obm2=bm2;
  }
}
...
void SetRenderMode(uint32_t DrawAttributes, BOOL bSCol)
{
 ...
 if(bDrawTextured)                                     // texture ? build it/get it from cache
 {
   ...
   if(gTexName!=currTex)
    {gTexName=currTex;glBindTextureStub(GL_TEXTURE_2D,currTex);}

   if(!bTexEnabled)                                    // -> turn texturing on
    {bTexEnabled=TRUE;glEnableStub(GL_TEXTURE_2D);}
  }
 else                                                  // no texture ?
 if(bTexEnabled) 
  {bTexEnabled=FALSE;glDisableStub(GL_TEXTURE_2D);}        // -> turn texturing off
...
void UploadScreenEx(int Position)
{
 ...
 glDisableStub(GL_BLEND);
 bBlendEnable=FALSE;
 glDisableStub(GL_TEXTURE_2D);
 bTexEnabled=FALSE;
 ...
 if(gdata&2) 
  {
   ...
   iDepthFunc=0;
   glDepthFuncStub(GL_LESS);
  }
 else
  {
   bCheckMask=FALSE;
   if(iDepthFunc==1) return;
   glDepthFuncStub(GL_ALWAYS);
   iDepthFunc=1;
  }
}
