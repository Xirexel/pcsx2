...
UINT64		   llAverageTimePerFrame = 0;
 ...
 if(bIsPerformanceCounter)
  {
   ...
   if(_ticks_since_last_update>(liCPUFrequency.LowPart>>4))   
    {
 ...
 if(dwActFixes&128)
  {
   ...
   llAverageTimePerFrame = (1.0f / fFrameRateHz) * 10000000.0f;
  }
 else
  {
   ...
   if(PSXDisplay.PAL)
    {
	   if (STATUSREG&GPUSTATUS_INTERLACED) {
		   ...
		   llAverageTimePerFrame = (677343.75f / 33868800.0f) * 10000000.0f;
	 }
	   else {
		   fFrameRateHz = 33868800.0f / 680595.00f;        // 49.76351
		   llAverageTimePerFrame = (680595.00f / 33868800.0f) * 10000000.0f;
	   }
    }                                                     
   else
    {
     if (STATUSREG&GPUSTATUS_INTERLACED)
	 {
		 ...
		 llAverageTimePerFrame = (565031.25f / 33868800.0f) * 10000000.0f;
	 }
	 else {
		 fFrameRateHz = 33868800.0f / 566107.50f;        // 59.82750
		 llAverageTimePerFrame = (566107.50f / 33868800.0f) * 10000000.0f;
	 }
    }
