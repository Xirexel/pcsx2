...
static __inline void DirectXdrawQuad(int y)
{
	...
	l_vertexes[2].x = iResX;
	l_vertexes[2].y = y + 1;
	l_vertexes[2].z = 0;
	l_vertexes[2].w = 1;


	draw(GL_QUADS, l_vertexes, FALSE);
 ...
 if(bOpaquePass)                                        // opaque mode?
  {
   ...
   TCF[1]=XP8RGBA_1;
   glAlphaFuncStub(GL_GREATER,0.49f);
  }
 else                                                  // no opaque mode?
  {
   TCF[0]=TCF[1]=P8RGBA;
   PalTexturedColourFn=P8RGBA;                         // -> init col func
   glAlphaFuncStub(GL_NOTEQUAL,0);                         // --> set alpha func
  }
   ...
   if(iScanBlend<0)                                    // special scan mask mode
    {
     ...
     glBindTextureStub(GL_TEXTURE_2D, gTexScanName);

     glTexImage2DStub(GL_TEXTURE_2D, 0, 4, 4, 4, 0,GL_RGBA, GL_UNSIGNED_BYTE, texscan);
    }
...
{   
 ...
 glScissorStub(0, 0, iResX, iResY);                        // init clipping (fullscreen)
...
clearBuffers(uiBufferBits);
 ...
 glEnableStub(GL_ALPHA_TEST);                              // wanna alpha test
 ...
 //if(bDrawDither)  glEnableStub(GL_DITHER);                 // dither mode
 //else             glDisableStub(GL_DITHER); 

 glDisableStub(GL_STENCIL_TEST);  
   ...
   if(iScanBlend<0)
    {
     if(gTexScanName!=0)                               // some scanline tex?
      glDeleteTexturesStub(1, &gTexScanName);              // -> delete it
     gTexScanName=0;
    }
  }
...
void SetOGLDisplaySettings(BOOL DisplaySet)
{
 ...
 if(bSetClip || !EqualRect(&r,&rC))
  {
   glScissorStub(r.left,r.top,r.right,r.bottom);
   rC=r;
   bSetClip=FALSE;
  }
}
