...
{
    CDVDNVMCallback(buffer, offset, bytes, read);
}
...
int cdvdReadSector()
{
    ...
    if (cdvd.BlockSize == 2064) {
        ...
        apex::MemoryManager::memcpy(&mdest[12], cdr.Transfer, 2048);
        ...
        apex::MemoryManager::memcpy(mdest, cdr.Transfer, cdvd.BlockSize);
    }
        ...
        switch (rt) {
            ...
            case 0x8D:            // sceMgWriteData
                ...
                if (cdvd.mg_size + cdvd.ParamC > cdvd.mg_maxsize) {
                    ...
                    apex::MemoryManager::memcpy(cdvd.mg_buffer + cdvd.mg_size, cdvd.Param, cdvd.ParamC);
            ...
            case 0x8E: // sceMgReadData
                ...
                apex::MemoryManager::memcpy(cdvd.Result, cdvd.mg_buffer, cdvd.ResultC);
                cdvd.mg_size -= cdvd.ResultC;
                apex::MemoryManager::memcpy(cdvd.mg_buffer, cdvd.mg_buffer + cdvd.ResultC, cdvd.mg_size);
                break;
            ...
            case 0x91: // sceMgReadBITLength
            {
                ...
                apex::MemoryManager::memcpy(cdvd.mg_buffer, &cdvd.mg_buffer[bit_ofs], 8 + 16 * cdvd.mg_buffer[bit_ofs + 4]);
